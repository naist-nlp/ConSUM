...
# def get_evaluator(self, task, max_length=1024, device='cuda:0', cache_dir=None):
...
class UniEvalScorer:
    def __new__(cls, task, max_length=1024, device='cuda:0', cache_dir=None):
        """ Set up unieval evaluator """
        assert task in ['summarization', 'dialogue', 'data2text', 'fact']
        if task == 'summarization':
            return SumEvaluator(max_length=max_length,
                                device=device,
    ...
    def evaluate(self, data, dims=None, overall=True, print_result=False):
        """
            ...
            dims: A list of dimensions to be evaluated. If dims is None, the evaluator will evaluate
                  all the default dimensions for the specific task.
